...
type ReplayPlayer = { steam_id: number; name: string; team: string; x: number; y: number; z: number; health: number; alive: boolean; yaw: number; duck_amount?: number | null };
type ReplayFrame = { tick: number; round: number; players: ReplayPlayer[]; visible_pairs: { observer: number; target: number }[] };
type ReplayData = { version: string; map: string; tick_rate: number; frames: ReplayFrame[]; approximate_spatial?: ApproximateSpatialRecord[] };
...
export default function ReplayViewer({ apiUrl, reportId, onClose }: { apiUrl: string; reportId: string | null; onClose: () => void }) {
  ...
  const coordinatePoint = (x: number, y: number) => ({ x: 8 + ((x - bounds.minX) / Math.max(bounds.maxX - bounds.minX, 1)) * 84, y: 90 - ((y - bounds.minY) / Math.max(bounds.maxY - bounds.minY, 1)) * 80 });
  const playerById = useMemo(() => new Map(frame?.players.map((player) => [player.steam_id, player])), [frame]);
  const approximateRecords = useMemo(() => functionalRecordsAtTick(replay?.approximate_spatial ?? [], frame?.tick ?? -1), [frame?.tick, replay?.approximate_spatial]);
  const crouchedPlayers = frame?.players.filter((player) => (player.duck_amount ?? 0) >= 0.5).length ?? 0;
  ...
  return <section className="replay-viewer" aria-label="Interactive Replay Viewer">
    ...
    {frame && replay ? <>
      <div className="replay-map" style={{ backgroundImage: "linear-gradient(rgba(8,22,21,.74), rgba(8,22,21,.89)), url('/manus-storage/sentinel-route-map_6c357b2d.jpg')" }}>
        ...
        {functionalGeometry ? <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="approximate-capsule-layer" aria-label="Functional approximate hitbox geometry">{approximateRecords.flatMap((record) => record.hitboxes.capsules.map((capsule) => { const start = coordinatePoint(capsule.start.x, capsule.start.y); const end = coordinatePoint(capsule.end.x, capsule.end.y); return <line key={`${record.player_id}-${capsule.name}-${capsule.group_id}`} x1={start.x} y1={start.y} x2={end.x} y2={end.y} />; }))}</svg> : null}
